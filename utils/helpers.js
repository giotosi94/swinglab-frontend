...
export const getRegimeColor = (r) =>
  ({ BULL: '#22c55e', NEUTRAL: '#eab308', BEAR: '#f97316', CRASH: '#ef4444' }[r] || '#94a3b8');

export const getSetupBadge = (type) => {
  const m = {
...
export const getSmartAlert = (asset) => {
  let factors = [];
  let conf = 0;
  ...
  if (asset.relative_volume >= 1.5) {
    ...
    factors.push({
      ...
      score: 1,
      max: 1,
      detail: asset.relative_volume?.toFixed(1) + 'x',
      pass: true,
    });
  } else {
    factors.push({
