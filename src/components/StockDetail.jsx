...
const text = (input, fallback = 'N/D') => {
  if (input == null) return fallback;
  if (typeof input === 'string' || typeof input === 'number') return input;
  if (typeof input === 'boolean') return input ? 'Si' : 'No';

  if (Array.isArray(input)) {
    const parts = input.map((item) => text(item, '')).filter(Boolean);
...
export default function StockDetail({ stock, onBack, onBuy, livePrice, mlScore, trendData }) {
  ...
  const phaseLabel = text(max.market_phase);
  ...
  const patterns = Array.isArray(stock.candlestick_patterns) ? stock.candlestick_patterns : [];
    ...
    <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              ...
              <Pill>{text(stock.sector_code)}</Pill>
              <Pill color={stock.data_status === 'FRESH' ? '#22c55e' : '#ef4444'}>{text(stock.data_status)}</Pill>
              <Pill color="#8b5cf6">Max live OFF</Pill>
            </div>
            <div style={{ color: '#64748b', fontSize: 11, marginTop: 5 }}>
              {text(stock.name, 'Analisi multi-timeframe Weekly, Daily e 4H')}
            </div>
          </div>

          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 26, fontWeight: 900 }}>{money(currentPrice)}</div>
            <div style={{ color: change >= 0 ? '#22c55e' : '#ef4444', fontWeight: 700 }}>
          ...
          {onBuy && (
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <input
                ...
                value={buyQty}
                min={1}
                onChange={(event) => setBuyQty(Math.max(1, parseInt(event.target.value, 10) || 1))}
                style={{
                  width: 62,
                  background: '#0f172a',
                  color: 'white',
              ...
              >
                {buyLoading ? 'Attendi' : `BUY ${stock.ticker}`}
              </button>
            </div>
          )}
        </div>
      ...
      <Section title="Max Strategy v1.5.2" accent="#8b5cf6">
        <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap', marginBottom: 10 }}>
          <Pill color={planColor}>{planStatus}</Pill>
          <Pill color="#8b5cf6">Fase titolo: {phaseLabel}</Pill>
          <Pill color="#06b6d4">{text(plan.execution_mode)}</Pill>
          <Pill color={max.strategy_eligible ? '#22c55e' : '#64748b'}>
            {max.strategy_eligible ? 'Strategy eligible' : 'Non eligible'}
          </Pill>
          <Pill color={max.trade_ready ? '#22c55e' : '#eab308'}>
            {max.trade_ready ? 'Trade ready' : 'In attesa'}
          </Pill>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(125px, 1fr))', gap: 8 }}>
          <Metric label="Max score" color="#8b5cf6">{text(max.max_score)}</Metric>
          ...
          <Metric label="Weekly">{text(weekly.structural_state ?? weekly.state)}</Metric>
          <Metric label="Daily">{daily.confirmed ? 'CONFERMATO' : text(daily.state, 'ATTESA')}</Metric>
          <Metric label="4H">
            {execution.available === false ? 'FALLBACK DAILY' : text(execution.state ?? plan.execution_mode)}
          </Metric>
          <Metric label="Ordine teorico">{text(plan.order_action, 'WAIT')}</Metric>
        </div>

        {Array.isArray(max.rejection_reasons) && max.rejection_reasons.length > 0 && (
          <div style={{ color: '#fca5a5', fontSize: 10, marginTop: 10 }}>
            Gate: {text(max.rejection_reasons)}
          </div>
        )}
      </Section>
      ...
      <Section title="Sintesi tecnica" accent="#3b82f6">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(110px, 1fr))', gap: 8 }}>
          <Metric label="Setup score" color={getScoreColor(stock.setup_score)}>{text(stock.setup_score)}</Metric>
          <Metric label="RSI">{stock.rsi != null ? Number(stock.rsi).toFixed(1) : 'N/D'}</Metric>
          ...
          <Metric label="Relative Volume">
            {stock.relative_volume != null ? `${Number(stock.relative_volume).toFixed(1)}x` : 'N/D'}
          </Metric>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(210px, 1fr))', gap: 10 }}>
          <div style={{ background: '#111827', borderRadius: 8, padding: 12 }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 900, marginTop: 5 }}>
              {mlScore?.ml_score != null ? `${mlScore.ml_score}%` : 'N/D'}
            </div>
            <div style={{ color: '#64748b', fontSize: 10 }}>
              {text(mlScore?.prediction, 'Informativo, non veto Max')}
            </div>
          </div>

          <div style={{ background: '#111827', borderRadius: 8, padding: 12 }}>
            <div style={{ color: '#94a3b8', fontSize: 10 }}>TREND 5 GIORNI</div>
            <div style={{ fontSize: 20, fontWeight: 900, marginTop: 5 }}>{text(trendData?.prediction)}</div>
            <div style={{ color: '#64748b', fontSize: 10 }}>
              Confidenza {trendData?.confidence != null ? `${trendData.confidence}%` : 'N/D'}
            </div>
          </div>

          <div style={{ background: '#111827', borderRadius: 8, padding: 12 }}>
            <div style={{ color: '#94a3b8', fontSize: 10 }}>WYCKOFF E ACCUMULO</div>
            <div style={{ fontSize: 16, fontWeight: 900, marginTop: 5 }}>{text(stock.wyckoff?.phase)}</div>
            <div style={{ color: '#64748b', fontSize: 10 }}>
              Accumulation score {text(stock.accumulation?.score)}
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section title="Approfondimenti legacy" accent="#475569">
        ...
        >
          {technicalOpen ? 'Nascondi dettagli' : 'Mostra EMA, MACD, pattern e analisi AI'}
        </button>

        {technicalOpen && (
          <div style={{ marginTop: 12, color: '#94a3b8', fontSize: 11, lineHeight: 1.65 }}>
            <div>
              EMA 10 / 20 / 50: {text(stock.ema10)} · {text(stock.ema20)} · {text(stock.ema50)}
            </div>
            <div>
              MACD: {text(stock.macd)} · Segnale: {text(stock.macd_signal)}
            </div>
            <div>
              Pattern: {patterns.length > 0 ? patterns.map((pattern) => text(pattern?.name, '')).filter(Boolean).join(', ') : 'Nessuno'}
            </div>
            {stock.llm_analysis && (
              <div style={{ marginTop: 8, padding: 10, background: '#111827', borderRadius: 7 }}>
                {text(stock.llm_analysis)}
              </div>
            )}
          </div>
        )}
        ...
        <Section title={`News (${newsData.news_count || newsData.news.length})`} accent="#06b6d4">
          {newsData.sentiment && (
            <div style={{ color: '#94a3b8', fontSize: 11, lineHeight: 1.55, marginBottom: 10 }}>
              {text(newsData.sentiment)}
            </div>
          )}

          {newsData.news.slice(0, 5).map((news, index) => (
            <button
              key={`${news.url || 'news'}-${index}`}
            ...
            >
              <div style={{ fontSize: 11, fontWeight: 700 }}>{text(news.headline, 'Notizia')}</div>
              <div style={{ fontSize: 9, color: '#64748b', marginTop: 3 }}>{text(news.source, '')}</div>
            </button>
          ))}
        </Section>
      )}
